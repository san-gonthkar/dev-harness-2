...
description: 'Autonomous phase orchestration for the Dev Harness. Use when running the V11 plan end-to-end: loop through every phase (P0-P10) autonomously, dispatch implementing agents, enforce review and coverage gates, recover from failures, and close phases until the entire plan is complete. Every invocation passes and updates the shared memory.md file.'
user-invocable: true
---
...
1. **Open a phase** — set it to `in progress` when you dispatch the first task.
2. **Update per task** — after each task, update the phase's task log (`pending` → `done`/`failed`) and the `Current Status` block.
3. **Mark blocked** — only after 3 failed attempts (retry + re-dispatch + escalate). Record the failure history and the decision needed.
4. **Close a phase** — only when all three phase-completion conditions are met:
   ...
   Then set the phase to `closed`, record the acceptance report path + `signed_by` + commit, and advance `Current Status` to the next phase.
