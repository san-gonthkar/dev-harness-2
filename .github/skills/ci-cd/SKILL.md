...
- Working with `.github/workflows/` — ci.yml, nightly.yml
- Setting up or debugging the PR/NIGHTLY/REL tiering
...
- **Mutation gates**: `scripts/mutation_gate.py` — re-run at release on the merged trunk, not only per-PR.
...
Each phase's `scripts/verify_phase_{NN}.sh` emits `reports/phase_{NN}_acceptance.json` with `verdict: ACCEPTED`. The implementing worker may not sign its own phase; phases 5, 8, 10 require a human signature.
