...
- A `PreToolUse` hook (`.github/hooks/stall-guard.json`) escalates on repeated identical tool calls (`ask` at 3, `deny` at 5) — the deterministic anti-loop guard.
- Cover branches in the module's own test file; do not add `*gaps*`/coverage-padding files. Use `parametrize` for contract tables.
...
- Dispatch 3-5 tasks per subagent brief (subagents run serially; per-task dispatch multiplies fixed startup cost).
- `memory.md` and `progress.md` are committed **once per phase close**, not per task. `progress.md`'s `AUTO:DASHBOARD` region is generated by `scripts/gen_dashboard.py` — never hand-edit it; `--check` fails on staleness.
