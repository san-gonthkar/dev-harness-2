...
- Deliverable: terminal degradation below 80x24. `is_degraded(w,h)` / `visible_panels(w,h) -> frozenset[PanelId]` are pure functions; below 80x24 only `PanelId.CANVAS` is visible, at/above all four. `HermesApp.on_mount` + `on_resize` call `_apply_responsive_layout`, which sets both `widget.display` (removes the panel from the grid) and `widget.visible` (so `widget.visible` reports the degradation — Textual's `visible` reads the CSS `visibility` rule, not `display`).
- Validation: `pytest tests/tui/test_responsive.py -q` -> **4 passed**, exit 0. Acceptance exact: (a) `run_test(size=(60,20))` mounts, no exception; (b) only `#execution-canvas` visible (other three not visible); (c) no exception. Regression: `tests/tui/test_layout.py` (100x30, all four visible) still green; full `tests/tui` -> **200 passed**.
- Tests: 4 (unit 2, integration 1, negative 1). No `time.sleep`, no network, no `engine/` import, no new deps, no state string literals. `mypy --strict` + `ruff check` clean.
- Unverified: `tui/` 75/65 package gate + full-suite coverage (smoke-only per brief; tracked requirement, not permission).
